...
| Capability | Status | Notes |
|---|---|---|
| Announce / broadcast a message to speakers and displays | next | `home.platform.traits.AssistantBroadcastTrait.BroadcastCommand` via `MeshInteractionService/SendCommands`, or a Foyer `GenAiHomeAgentService/ProcessQuery` text query; local Cast fallback |
...
| Device on/off, brightness, colour temperature, colour | planned | `HomeControlService/UpdateTraits`; trait payloads documented by googlehome-mcp |
| Volume / mute, media play-pause-stop | planned | `UpdateTraits` `volume` / `mediaState` |
| Full device state read | planned | `devices state` over `GetTraits` (all traits, not just online) |
...
| Raw RPC passthrough | done | `api POST <Service>/<Method> --data '[...]'` |
